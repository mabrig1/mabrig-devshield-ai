...
export async function validateRuntimeTarget(raw, { allowHttp = false, allowPrivate = false, lookupFn = lookup } = {}) {
  ...
  if (!allowPrivate) {
    if (net.isIP(h)) {
      if (isPrivateIp(h)) throw new Error('runtime-target points to a private or non-routable IP');
    } else {
      const addresses = await lookupFn(h, { all: true, verbatim: true });
      if (!addresses?.length) throw new Error('runtime-target hostname did not resolve');
      if (addresses.some(x => isPrivateIp(x.address))) throw new Error('runtime-target resolves to a private or non-routable IP');
    }
  }
  return u;
}
...
async function hit(url, { fetchFn, timeoutMs, authHeader, vercelBypassToken }) {
  const headers = { 'user-agent': 'MABRIG-DevShield-Runtime-Guard/2.4', 'x-devshield-runtime-probe': '1' };
...
function sarif(r) {
  return { version: '2.1.0', $schema: 'https://json.schemastore.org/sarif-2.1.0.json', runs: [{
    tool: { driver: { name: 'MABRIG DevShield Runtime Guard', version: '2.4.0' } },
...
export async function runRuntimeGuard(options = {}) {
  const {
    ...
    timeoutMs = 8000, fetchFn = globalThis.fetch, lookupFn = lookup, workspace = process.cwd(),
    reportDir = '.devshield', writeSarif = true, authHeader = '', vercelBypassToken = '',
    outputFile = '', summaryFile = ''
  } = options;
  if (typeof fetchFn !== 'function') throw new Error('Runtime Guard requires fetch');
  const validated = await validateRuntimeTarget(target, { allowHttp, allowPrivate, lookupFn });
  let baseline;
  try { baseline = await hit(validated, { fetchFn, timeoutMs, authHeader, vercelBypassToken }); }
  catch (e) { baseline = { status: null, error: e?.message || String(e) }; }
  const probes = [];
  if (![401, 403].includes(baseline?.status) && baseline?.status != null) {
    for (const p of RUNTIME_PROBES) {
      try { const r = await hit(probeUrl(validated, p), { fetchFn, timeoutMs, authHeader, vercelBypassToken }); probes.push({ ...p, status: r.status, outcome: outcome(r.status) }); }
      catch (e) { probes.push({ ...p, status: null, outcome: 'error', error: e?.message || String(e) }); }
    }
  }
  ...
  const report = {
    schemaVersion: 1, tool: 'MABRIG DevShield Runtime Guard', version: '2.4.0',
