...
export const rules = [
  ...
  rule('workflow-unsafe-pr-checkout', 'high', 'ci-security', /^\s*allow-unsafe-pr-checkout\s*:\s*true\s*$/i, 'Workflow explicitly opts out of GitHub pull-request checkout protection.', 'CWE-829', 'Remove allow-unsafe-pr-checkout or ensure contributor-controlled code is treated only as data and never executed in a privileged workflow.', { file: /\.github\/workflows\/.*\.ya?ml$/i }),
